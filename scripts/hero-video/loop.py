#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""首屏循环:绿球从前景两人的手机里跳出,飞向场内的其他人。

## 为什么是一条片子而不是三条剪辑

三镜版在循环处很生硬,而生硬的根源不是剪辑手法 —— 是**前后拍的不是同一件事**:
第一镜是背影,第三镜是正脸,跳回开头就像换了个场景。全程用背影这一条,就没有
接缝可言,因为根本没有剪辑。

前景那两个人不抬头、不转身、什么都不做。动的只有那颗球。
...
SRC = os.path.expanduser("~/ComfyUI-H3/output/peerproof_scene2/e_00001_.mp4")
FRAMES = "/tmp/se"
OUT = "/tmp/loop"
REF = 0                       # 目标坐标是在第 1 帧上读的
ORIGIN = (480, 527)           # 左边那人的手机
# 分裂点。两人的头顶在 y≈360,所以球升到 250 是明显高过他们,但不进大屏那块最亮的
# 区域 —— 在近白背景上,加法混合再绿也是白。
APEX = (515, 250)
TARGETS = [(75, 400), (140, 392), (352, 368), (690, 366), (845, 358)]

RISE_FROM, RISE_TO = 4, 26    # 升起
FLY_FROM, FLY_TO = 26, 60     # 分裂之后一起飞
...
# 两头在画面里,中间那截看不见。
# 高出弦的峰值,不是从端点起算的偏移。外侧路程长,抛得高;中间路程短,抛得矮 ——
# 全都一样高的话,五条弧会在同一高度到顶,叠成一条横带。
ARCS = [120, 96, 62, 88, 118]
...
def bezier(a, b, t, arc):
    ...
    上一版把控制点放在「两端中较高的那个再往上抬 arc」,那个点不在中间,画出来是
    一条歪掉的曲线;为了让球落到人身上又把 arc 砍到 8-22 像素,于是它彻底变成了
    一条斜直线 —— 没有先扬后落,自然不像抛的。

    这里用的是弦加下垂项:水平方向匀速,垂直方向在弦的基础上减去 4H·t(1-t)。
    这一项在 t=0.5 处取最大值 H,两端为零 —— 也就是说峰值一定在正中间、一定高出
    弦 H 像素,和起点终点谁高谁低无关。这就是抛体运动的形状。

    水平用 t^0.88 而不是 t:真实的抛物飞行里,落地前那一段看着是慢下来的(视觉上
    离得远、角度变化小),稍微减速更像被抛出去而不是被拖过去。"""
    tx = t ** 0.88
    x = a[0] + (b[0] - a[0]) * tx
    y = a[1] + (b[1] - a[1]) * tx - 4 * arc * tx * (1 - tx)
    return (x, y)
...
def main():
    ...
    for i, f in enumerate(files):
        ...
        # 光带自己的「遮挡」层。一个发光的实体会挡住身后的东西,而纯 screen 不会 ——
        # 所以球飞过那块近白的大屏时,加多少绿都还是白(加法叠在白上永远是白)。
        # 先按这一层把背景局部压暗,绿色才有地方落。
        shade = np.zeros(img.shape[:2], np.float32)
        M = Ms[i]
        o = warp(ORIGIN, M)
        ...
        def spark(center, r, color, strength=1.0):
            ...
            # 三层的强度整体压低。球会飞过大屏那块近白的区域,而 screen 是加法 ——
            # 在接近白的背景上再加满强度的光,出来只能是白,绿色全丢。压低之后它是
            # 「亮绿」而不是「过曝」。
            cv2.circle(shade, (cx, cy), max(1, int(r * 1.5)), float(strength), -1, cv2.LINE_AA)
            for mul, a in ((3.2, 0.10), (1.8, 0.22), (1.0, 0.78)):
                v = a * strength
        ...
        def streak(path_at, t, width0, seed=0):
            ...
            for j in range(len(pts) - 1):
                ...
                cv2.line(shade, pts[j], pts[j + 1], float(fade), max(1, int(w * 1.4)), cv2.LINE_AA)
        ...
        glow = cv2.GaussianBlur(glow, (0, 0), 9) * 0.55 + glow
        shade = cv2.GaussianBlur(shade, (0, 0), 7)[..., None]
        # 先压暗、再发光。压暗是为了让绿色在亮背景上也读得出来 —— 0.62 是试出来的:
        # 再深就像画面破了个洞,再浅绿色又被白吃掉。
        out = img * (1 - np.clip(shade, 0, 1) * 0.62)
        out = 255 - (255 - out) * (255 - glow) / 255
