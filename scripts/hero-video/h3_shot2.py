#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""第二镜:一个人低头操作手机(原 h3_scene2 的骨架 —— 镜头面向大屏幕,前景两个背影,人群稀疏。
...
OUT = os.path.expanduser("~/ComfyUI-H3/output/peerproof_shot2")
...
SHOTS = [
    ("g", 7101, STYLE +
     "Medium close shot inside the same dark event hall at night. ONE young attendee stands among "
     "the crowd with their head lowered, both hands cradling their phone, thumbs moving as they "
     "tap. The phone's glow lights their face and hands from below. The huge stage screen behind "
     "throws a violet rim light across their hair and shoulders; the crowd around them is blurred "
     "far out of focus. The camera is behind and to the side of their shoulder, so the phone's "
     "screen is never visible. " + BRIGHT + ALIVE_LOCKED + NO_TEXT),
    ("h", 7202, STYLE +
     "Close shot from over the shoulder of one young attendee at a night event, seen from behind "
     "and slightly above. They hold a phone in both hands at chest height and tap it; we see the "
     "back of the phone and their hands, never the screen. Purple stage light rakes across their "
     "shoulders from the bright screen ahead, haze in the beams, blurred silhouettes of other "
     "people around them. " + BRIGHT + ALIVE_LOCKED + NO_TEXT),
    ("i", 7303, STYLE +
     "Medium shot of a young attendee standing in a bright, hazy event hall at night, phone held "
     "in both hands, face lit from below by its glow, eyes down on it, a small smile. Other "
     "attendees pass behind them completely out of focus. The phone is angled away so its screen "
     "is not in frame. " + BRIGHT + ALIVE_LOCKED + NO_TEXT),
]
...
def build(prompt, seed, tag):
    return {
        ...
        "43": {"class_type": "SaveVideo", "inputs": {
            "video": ["42", 0], "filename_prefix": f"peerproof_shot2/{tag}", "format": "mp4", "codec": "h264"}},
    }
...
def main():
    ...
    for tag, seed, prompt in SHOTS:
        ...
        if not done:
            ...
            bad += 1
    print("SHOT2_DONE" if bad == 0 else f"SHOT2_PARTIAL 坏 {bad} 条", flush=True)
