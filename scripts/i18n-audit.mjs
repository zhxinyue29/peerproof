...
for (const f of walk(join(WEB, "app")).concat(walk(join(WEB, "components")), walk(join(WEB, "lib")))) {
  ...
  for (const m of s.matchAll(/"((?:nav|common|events|organizer|floor|venue|event|verify|home|identity|error|gate|scan|timeline|create|deploy|topup|myEvents|graph|lang|strip|scene|art|payout|registered|myEvents|stats|footer|proof|live)\.[\w.-]+)"/g)) {
    if (!used.has(m[1])) used.set(m[1], []);
    used.get(m[1]).push(f.replace(WEB + "/", ""));
  }
}
