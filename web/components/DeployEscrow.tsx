  ...
  return (
    <div className="space-y-3 rounded-xl border border-dashed border-warn/40 p-4">
      ...
      {result ? (
        ...
        <div className="space-y-2">
          <Notice tone="warn">{t("deploy.escrowWarning")}</Notice>
          <div className="flex gap-2">
            <Button
              onClick={() => {
                setBusy(true);
                setError(null);
                void walletDeploy(signer.address, attendanceEscrowBytecode, 2_800_000n)
                  .then(setResult)
        ...
        <>
          ...
          <p className="text-[11px] leading-relaxed text-faint">
            About 0.29 MON. The limit is pinned at 2,800,000 rather than estimated, because Monad
            charges the limit and not the amount used — the deployment measures 2,370,395, and the
            rest is headroom against a failure that would be billed the same and have to be paid
            twice.
          </p>
        </>
      )}
